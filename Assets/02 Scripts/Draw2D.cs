...
2. 드로잉 툴에서 멀찍이 떨어진 거리에서 그림이 그려짐 (해결)
*/
...
public class Draw2D : MonoBehaviour
{
    ...
    GameObject _top, _center, _bottom;
    ...
    Vector3 _baseTop, _baseCenter, _baseBottom; //기준점이 되는 top, center, bottom의 
    ...
    void CreateNewMesh()
    {
        if (_isNew && OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch))
        {
            ...
            //Material을 새로 생성하고 싶다. 2DBrush의 경우 defaultMat형식의 Material을 생성한다.
            Material mat = new Material(defaultMat);
            mat.color = ColorPickerToUse.value;
            //생성한 Material을 MeshRender에 할당해준다.
            ...
            //그리기 시작할 때의 vertex는 두개다.
            vertexList.Add(_top.transform.position);
            vertexList.Add(_bottom.transform.position);

            //작업 중인 메쉬에 우선적으로 이 두 점을 넣어준다.
    ...
    /// <summary>
    /// 기준center와 현재 center의 거리를 재는 함수. 거리값을 반환. DrawMesh()에서 쓰인다
    /// </summary>
    /// <returns></returns>
    ...
    void NewBasePoints()
    {
        ...
        _baseBottom = new Vector3(_bottom.transform.position.x, _bottom.transform.position.y, _bottom.transform.position.z);
    }
    ...
    void DrawMesh()
    {
        ...
        if (_isDrawing && currentDistance > unitDistance) //기준 거리 초과되면!
        {
            if (workingMeshObj != null && workingMesh != null)
            {
                ...
                //_baseTop-currentTop-_baseBottom 순서 triangle 하나와,
                triangles.Add(vertexList.Count - 4);
                triangles.Add(vertexList.Count - 2);
                triangles.Add(vertexList.Count - 3);

                //currentTop - currentBottom - _baseBottom 순서 triangle 하나를 그린다
                triangles.Add(vertexList.Count - 2);
